...
PREFIX = "30pf10sim_"								#Map prefix used in the original sim
...
FINALYEAR = 500										#Number of the last year to include in the analysis
...
for iteration in range(iterations):
	...
	elevmaps = grass.read_command('g.mlist', flags='m', type='rast', pattern=namestem, separator=',').strip().split(',')
	cumerdeplist = []
	...
	np.savetxt("Year_%s_%s.csv" % (str(looper).zfill(DIGITS), OUTPUTFILE), statsarray.T, delimiter=",")
################################
	if ERRORCALC is True:
		...
		np.savetxt("Year_%s_%s_errors.csv" % (str(looper).zfill(DIGITS), OUTPUTFILE), errorarray.T, delimiter=",")
###############################
	if looper == FINALYEAR:
	  break
	else:
